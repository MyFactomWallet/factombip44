...
describe('FactomHDWallet', function () {
  
  it('Using a seed', function () {
    const seed = bip39.mnemonicToSeedHex('yellow yellow yellow yellow yellow yellow yellow yellow yellow yellow yellow yellow')
    const wallet = new bip44.FactomHDWallet({ seed })
  ...
  it('List from http://stevenmasley.me/bip39.html implmentation for salted mnemonic seed', function () {
    ...
    for (let i = 0; i < 5; i++) {
      const wallet = new bip44.FactomHDWallet({mnemonic: mn, passphrase})
      const addr = fctUtils.seedToPrivateFctAddress(wallet.generateFactoidPrivateKey(0, 0, i))
